...
class ProductTemplate(models.Model):
    ...
    def set_multicompany_taxes(self):
        self.ensure_one()
        tax_ids = self.taxes_id.ids
        supplier_tax_ids = self.supplier_taxes_id.ids
        company_id = self.env.user.company_id.id

        self = self.sudo()
        default_taxes = self.taxes_by_company('taxes_id', company_id)
        default_supplier_taxes = self.taxes_by_company(
            'supplier_taxes_id', company_id)
        # Use list() to copy list
        match_tax_ids = default_taxes != tax_ids and list(tax_ids) or False
        match_suplier_tax_ids = (default_supplier_taxes != supplier_tax_ids and
                                 list(supplier_tax_ids) or False)
        for company in self.env['res.company'].search(
                [('id', '!=', company_id)]):
            tax_ids.extend(self.taxes_by_company(
                'taxes_id', company.id, match_tax_ids))
